...
public class MemoryOrderZonesView extends LinearLayout {

    public interface OnOrderChangedListener {
        void onOrderChanged(String order, boolean java, boolean nativeHeap, boolean graphics, boolean total);
    }
    ...
    public MemoryOrderZonesView(Context context) {
        this(context, null);
    }

    public MemoryOrderZonesView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void setOrder(String order) {
        ...
        if (order == null || order.isEmpty()) {
            order = "java,native,graphics,total";
        }
    ...
    private void finishDrag(float rawX, float rawY) {
        ...
        MemoryConfig.showJavaHeap = activeIds.contains("java");
        MemoryConfig.showNativeHeap = activeIds.contains("native");
        MemoryConfig.showGraphics = activeIds.contains("graphics");
        MemoryConfig.showTotal = activeIds.contains("total");
        MemoryConfig.itemOrder = buildOrder();

        if (listener != null) {
            listener.onOrderChanged(MemoryConfig.itemOrder,
                    MemoryConfig.showJavaHeap,
                    MemoryConfig.showNativeHeap,
                    MemoryConfig.showGraphics,
                    MemoryConfig.showTotal);
        }
    }
    ...
    private boolean isActive(String id) {
        switch (id) {
            case "java":
                return MemoryConfig.showJavaHeap;
            case "native":
                return MemoryConfig.showNativeHeap;
            case "graphics":
                return MemoryConfig.showGraphics;
            case "total":
                return MemoryConfig.showTotal;
            default:
                return true;
        }
    }
